def DJ_diff_alpha_algorithm(error_threshold,timeseries_data,alpha_in_buf,alg):   
    '''
    Input:
    ...
    import numpy as np
    import math
    from DJwithconvexhall import DJ_algorithm
    ...
    while interval_start < len(timeseries_data)-1:
        ...
        if alg == 'p':
            ...
            et_interval = et*math.exp(0.5-max_min_in_interval/max_min_in_timeseries_total)
        elif alg == 'o':
            et_interval = et
        
        # do DJ
        segment_interval = DJ_algorithm(et_interval,interval,max_min_in_timeseries_total)
        ...
        interval_start = int(segment_total[-1][1]+1) #s_end of the last segment + 1

    if interval_start == len(timeseries_data)-1: # interval before the last one has only 1 segment and stops at 'timeseries_total.count()-2'
        segment_total.append([len(timeseries_data)-1, len(timeseries_data)-1, len(timeseries_data)-1, timeseries_data[len(timeseries_data)-1], 0])    
